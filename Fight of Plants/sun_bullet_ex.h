...
public:
	...
	{
		bool is_collide_x = (max(this->position.x + this->size.x, 
			position.x + size.x) - min(this->position.x, position.x) 
			<= this->size.x + size.x); 
		bool is_collide_y = (max(this->position.y + this->size.y,
			position.y + size.y) - min(this->position.y, position.y) 
			<= this->size.y + size.y);
	...
	~SunBulletEx() = default;
private:
	Animation animation_idle;//æﬁ¥Û»’π‚’®µØƒ¨»œ∂Øª≠
	Animation animation_explode;// æﬁ¥Û»’π‚’®µØ±¨’®∂Øª≠
	Vector2 explode_render_offset;//±¨’®∂Øª≠‰÷»æ∆´“∆
};
